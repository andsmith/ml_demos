...
def plot_dataset(ax, x, y, *args, **kwargs):
    ...
    ax.plot(x[y==labels[0], 0], x[y==labels[0], 1], '.', color='red',**kwargs)
    ax.plot(x[y==labels[1], 0], x[y==labels[1], 1], '.', color='blue',**kwargs)


def plot_classifier(ax, x, y, model, boundary, res=500, weights=None,bubble_size=30, invert_incorrect_colors=False, **kwargs):
    ...
    false_pos = np.where((y_hat == 1) & (y == 0))[0]
    false_neg = np.where((y_hat == 0) & (y == 1))[0]
