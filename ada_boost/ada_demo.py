...
from perceptron import Perceptron, DecisionStump
from spiral import make_bump, make_spiral_data
import logging
...
class AdaDemo(object):
    ...
    def __init__(self, n_side_pts=25, n_rows=2):
    ...
    def _plot_ensemble(self,row, iter):
        ...
        ax.set_title('Ensemble at iteration {}, accuracy:  {}'.format(iter, accuracy))
    ...
    def _plot_weak(self, new_model, w_loss, row, iter):
        ...
        ax.set_title('Weak learner, iteration {}, wtd.loss {}'.format(iter, w_loss))
    ...
    def _plot_boundary(self, ax):
        ...
        ax.contourf(xx, yy, preds, levels=[-1, 0, 1], alpha=.2)

    def _plot_dataset(self, ax):
        ax.plot(self._points[self._labels == 0, 0], self._points[self._labels == 0, 1],
                 'b.', markersize=POINT_MARKER_SIZE)
        ax.plot(self._points[self._labels == 1, 0], self._points[self._labels == 1, 1],
                 'r.', markersize=POINT_MARKER_SIZE)
    ...
    def _run(self):
        ...
        while True:
            ...
            logging.info('Iteration {}: w_loss = {}, loss = {}, g_loss= {}, alpha = {}'.format(self._iter, weighted_loss, self._loss[-1], ,alpha))
