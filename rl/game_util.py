...
from node_placement import FixedCellBoxOrganizer, CompactBoxOrganizer, FixedCellWithKey  # for 2d embedding and vertical sorting
import cv2
...
def sort_states_into_layers(state_list, player_mark=Mark.X, key='id'):
    ...
    for n_marks in range(6):
        ...
        layers.append(layer)
        logging.info("Layer %i has %i states." % (n_marks, len(layer)))
    return layers
